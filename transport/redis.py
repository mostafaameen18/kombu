...
class Channel(virtual.Channel):
    ...
    def _connparams(self, asyncronous=False, _r210_options=(
    ...
    def _create_client(self, asyncronous=False):
        if asyncronous:
            return self.AsyncClient(connection_pool=self.async_pool)
        return self.Client(connection_pool=self.pool)

    def _get_pool(self, asyncronous=False):
        params = self._connparams(asyncronous=asyncronous)
    ...
    def async_pool(self):
        if self._async_pool is None:
            self._async_pool = self._get_pool(asyncronous=True)
        return self._async_pool

    @cached_property
    def client(self):
        """Client used to publish messages, BRPOP etc."""
        return self._create_client(asyncronous=True)

    @cached_property
    def subclient(self):
        """Pub/Sub connection used to consume fanout queues."""
        client = self._create_client(asyncronous=True)
